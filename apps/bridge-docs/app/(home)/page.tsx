    ...
    <main className="flex flex-1 flex-col items-center justify-center text-center px-4 py-16">
      <div className="max-w-3xl mx-auto">
        <div className="inline-flex items-center gap-2 rounded-full border border-fd-border px-4 py-1.5 text-sm text-fd-muted-foreground mb-6">
          Cross-Chain · MPC-Secured · Multi-Network
        </div>
        <h1 className="text-5xl md:text-7xl font-bold mb-6 bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
          Lux Bridge
        </h1>
        <p className="text-lg text-fd-muted-foreground mb-8 max-w-2xl mx-auto">
          Cross-chain bridge with MPC-secured transfers.
          Move assets between Lux subnets, Ethereum, Bitcoin, and more.
        </p>
        <div className="rounded-lg border border-fd-border bg-fd-card p-4 mb-8 max-w-md mx-auto font-mono text-sm">
          <span className="text-fd-muted-foreground">$</span>{' '}npm install @luxfi/bridge
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-left">
          {[
            { name: 'Cross-Chain Transfers', desc: 'Bridge assets between any supported chain' },
            { name: 'MPC Security', desc: 'Threshold ECDSA signing with HSM attestation' },
            { name: 'Relay Network', desc: 'Decentralized relay with consensus verification' },
            { name: 'Multi-Chain', desc: 'Lux subnets, Ethereum, Bitcoin, and more' },
            { name: 'Bridge UI', desc: 'Production-ready Next.js frontend' },
            { name: 'Bridge API', desc: 'REST API for programmatic transfers' },
          ].map((c) => (
