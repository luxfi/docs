...
ETHEREUM_JSONRPC_HTTP_URL: "http://luxd-1.luxd-headless.lux-mainnet.svc:9630/v1/chain/{blockchain_id}/rpc"
```
...
For sovereign L1 chains with small block counts, use the memory-saving environment variables from the [Indexing](/explorer/backend/indexing) page. Set container memory limit to 8Gi with `Recreate` strategy.
