...
For subnet chains with small block counts, use the memory-saving environment variables from the [Indexing](/explorer/backend/indexing) page. Set container memory limit to 8Gi with `Recreate` strategy.
