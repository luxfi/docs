...
The Go SDK provides typed wrappers around all Lux node JSON-RPC endpoints. Each chain type (P, X, C, L2 EVM) has its own client.
...
L2 EVM chains use the same `ethclient` interface with a different endpoint:
