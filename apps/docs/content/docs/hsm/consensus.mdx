...
The `github.com/luxfi/hsm` package provides HSM integration for Lux consensus threshold signing, supporting BLS, FROST, CGGMP21, and Pulsar (lattice-based post-quantum) protocols.

## Architecture

Lux uses dual-threshold consensus (Quasar):
- **BLS** (1 round) — classical aggregatable signatures for fast finality
- **Pulsar** (2 rounds) — lattice-based threshold for post-quantum security

Both run in parallel. BLS provides 500ms block finality; Pulsar creates quantum-safe bundles every 3 seconds.
...
import (
    ...
    _ "github.com/luxfi/corona/threshold"        // Register Pulsar
)
...
// Store Pulsar key share (post-quantum)
...
// Store both BLS and Pulsar shares
...
| Pulsar | 2 | Yes | Quantum-safe bundles |
...
| Post-quantum readiness | Pulsar shares in same vault |
