...
Validators need reliable hardware with high uptime. Use the same requirements as a [full node](/nodes/full-node), with emphasis on:
...
<Steps>
  <Step>
    ...
    Your node must be fully bootstrapped before you can register as a validator. Follow the [full node guide](/nodes/full-node) and wait until `info.isBootstrapped` returns `true` for all chains.
...
- [Full Node Guide](/nodes/full-node) - Node installation and configuration
- [Tokenomics](/learn/tokenomics) - Detailed reward economics
- [Platform API](/reference/platform) - Manage validators programmatically
