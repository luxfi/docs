...
curl -s -X POST --data '{
  ...
  "params": []
}' -H 'content-type:application/json' http://localhost:9650/v1/bc/C/rpc
```
...
- [Full Node Guide](/nodes/full-node) - Base node setup
- [JSON-RPC API](/api-reference/json-rpc) - Query endpoints available on archive nodes
- [Health API](/api-reference/health) - Monitor archive node health
