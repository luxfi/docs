...
description: "Post-quantum cryptographic operations: Corona signatures, Quasar consensus, parallel verification."
icon: Atom
---

The Quantum VM (Q-Chain) is a post-quantum resistant blockchain virtual machine. It provides quantum-safe signatures using Corona keys, parallel transaction processing, and bridges classical BLS consensus with post-quantum Corona threshold signatures via the Quasar consensus protocol. The Q-Chain uses DAG-based consensus for high-throughput parallel processing.

## Architecture

- **Corona Keys**: Post-quantum key pairs (default 1024 bytes) with version tracking
...
- **Quasar Bridge**: Hybrid consensus connecting P-Chain BLS with Q-Chain Corona threshold signatures
...
| `coronaKeySize` | 1,024 | Corona key size in bytes |
...
- **qvm.generateCoronaKey** -- Generate a new Corona key pair (returns public key, version, size)
