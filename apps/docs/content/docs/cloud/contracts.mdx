...
<Tabs items={["Mainnet", "Testnet"]}>
  ...
  <Tab value="Testnet">
    ...
    RPC URL:      https://api.lux-test.network/v1/chain/c
    Chain ID:     96368
    Symbol:       LUX
...
<Steps>
  ...
  <Step>
    ### Deploy with Forge
    ```bash
    forge create --rpc-url https://api.lux-test.network/v1/chain/c \
...
<Callout>
  Precompile access is governed by admin roles configured at the chain level. On the Primary Network C-Chain, most precompiles are restricted to governance operations.
</Callout>
...
- [Tutorials](/cloud/tutorials) - Step-by-step contract deployment guides
- [JSON-RPC API](/api-reference/json-rpc) - Interact with deployed contracts via RPC
- [SDKs](/cloud/sdks) - Call contracts from JavaScript, Go, or Rust
