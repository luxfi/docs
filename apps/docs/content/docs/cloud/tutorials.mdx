...
Deploy your own chain with a custom gas token and fee structure.
...
<Steps>
  ...
  <Step>
    ### Deploy Locally
    ```bash
    lux chain deploy game-chain --local
    ```

    The CLI will output your chain's RPC URL and chain ID.
  </Step>
  <Step>
    ### Connect with MetaMask

    Add the custom network to MetaMask:
    - **RPC URL**: Provided by `lux chain deploy` output
    - **Chain ID**: 99999
    - **Symbol**: GAME
  </Step>
  <Step>
    ### Deploy Contracts to Your Chain
    Update your Hardhat config with the local chain RPC and deploy as usual:

    ```bash
    npx hardhat run scripts/deploy.js --network localChain
    ```
  </Step>
</Steps>
...
- [Smart Contracts](/cloud/contracts) - Contract development reference
- [SDKs](/cloud/sdks) - Interact with contracts programmatically
- [Chains](/cloud/chains) - Full chain configuration reference
