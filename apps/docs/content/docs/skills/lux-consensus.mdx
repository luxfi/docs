...
|--------|------|----------|---------|--------|
| **Quasar** | Hybrid PQ | `protocol/quasar/` + `node/consensus/quasar/` | Primary consensus for all chains. Dual BLS+Corona signatures run in parallel. 2-round finality. | Production |
...
type CertBundle struct {
    ...
    PQCert []byte  // ~KB Corona threshold signature
}
...
| **Q-day (BLS broken)** | Attacker can forge BLS but NOT Corona lattice signatures. Block fails quantum check. Consensus halts rather than accepting unsafe fork. |
| **Post-quantum** | Security rests on lattice SVP hardness (2^160 operations). Corona threshold signatures remain secure. |
| **Attack window** | <= PQ round time (<=50ms on mainnet). |

Both BLS and Corona proofs are **REQUIRED** for Q-Chain validators. No fallback mode. The `Verify` method enforces strict dual-proof validation.
...
consensus/
  protocol/
    ...
    quasar/         Post-quantum finality engine
      quasar.go     Signer (BLS+Corona dual threshold)
...
5. **Finality proof**: Signed with Quasar hybrid signatures (BLS + Corona).
...
| Quantum adversary (BLS breakable) | Safe | Corona lattice certificates prevent acceptance |
