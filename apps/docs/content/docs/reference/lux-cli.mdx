...
| `luxfi/evm` | v0.8.35 | EVM/Sovereign L1 EVM configuration |
...
lux
  chain          Blockchain lifecycle (create/deploy/list/describe/delete/import)
    create       Create new blockchain configuration (L1/L3)
  ...
  key            Manage cryptographic keys
    create       Generate new key set from mnemonic (EC, BLS, Corona, ML-DSA)
...
| Corona | Lattice-based ring | Ring signatures for privacy |
...
NEVER use well-known test keys. Always generate fresh keys with `lux key create`.
...
Reports availability for: NTT operations (Corona consensus), FHE operations (ThresholdVM), lattice cryptography. Builds with CGO for GPU support (`cgo_enabled.go` / `cgo_disabled.go`).
