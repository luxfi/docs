...
## ZAP Consensus Transport

**ZAP** (`github.com/luxfi/zap`) is the primary transport for consensus messages between validators. ZAP provides zero-copy binary serialization (17x faster than JSON-RPC, 2.9 ns parse time, 0 allocations) over TCP with optional TLS.

### PQ-TLS 1.3

All node-to-node connections use PQ-TLS 1.3 for transport security. Go 1.26 defaults to **ML-KEM-768 + X25519** hybrid key exchange in `crypto/tls`, so PQ-TLS is enabled without additional configuration. Key properties:

- **Key exchange**: X25519MLKEM768 hybrid (NIST Level 3 + classical ECDH)
- **Configuration**: `NodeConfig.TLS` field on the ZAP node
- **Zero-copy preserved**: TLS does not break ZAP zero-copy semantics -- the TLS layer operates on the same buffer without additional heap allocations
- **Mutual auth**: Validators authenticate via TLS client certificates (node staking cert)
