...
Lux Explore is the **Next.js frontend** for the Lux Network block explorer. Originally a fork of the Blockscout frontend (now archived at `github.com/luxfi/explorer-v1`), it provides the web UI for browsing transactions, contracts, tokens, and network activity at `explore.lux.network`.
...
| Node |  &gt;= 22.14.0 |
| Upstream | `blockscout/frontend` (diverged) |
| License | GPL v3.0 |
...
NEXT_PUBLIC_NETWORK_RPC_URL=https://api.lux.network/v1/chain/c
...
Key areas where Lux branding diverges from the upstream fork:
...
- `lux/lux-explorer.md` -- Explorer backend (Go)
