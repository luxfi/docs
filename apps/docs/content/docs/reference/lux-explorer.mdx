...
The `EthereumJSONRPC` app connects to Lux nodes via EVM-compatible endpoints:
...
Each Lux chain has a unique blockchain ID (e.g., `dnmzhuf6poM6PUNQCe7MWWfBdTJEnddhHRNXz2x7H6qSmyBEJ` for LUX mainnet C-Chain).
...
- Running Lux node (luxd) with EVM
