...
Lux Quasar is the consensus engine for the Lux network. It is a unified, leaderless, post-quantum protocol achieving sub-second 2-round finality with triple-certificate security (BLS12-381 aggregate signatures + Corona lattice-based threshold signatures + ML-DSA-65 identity signatures). All three paths run in parallel via `TripleSignRound1`. Quasar runs every chain in the Lux primary network -- Q, C, X, P -- and any dynamically registered chain.
...
- **Node integration** (`github.com/luxfi/node/consensus`): The node-level wiring that binds Quasar to P-Chain finality events, the ZAP agentic bridge, and Corona coordinator lifecycle.
...
|--------|------|----------|---------|--------|
| **Quasar** | Triple PQ | `protocol/quasar/` + `node/consensus/quasar/` | Primary consensus for all chains. Triple BLS+Corona+ML-DSA signatures run in parallel via `TripleSignRound1`. 2-round finality. | Production |
...
| **PQ engine** | Post-quantum | `engine/pq/` | Post-quantum engine variant. Chain engine with `QuantumResistant=true` config enabling Corona certificates on every block. | Production |

## Quasar Algorithm -- Detailed Steps

Quasar achieves triple quantum finality through parallel execution of classical and post-quantum signature paths (BLS + Corona + ML-DSA). The full algorithm:
...
### Phase 2: QuasarCert Generation (Quantum Finality)

All three paths execute **in parallel** immediately after classical decision (via `TripleSignRound1`):
...
**Corona Path (quantum-safe, 2-round):**
1. **Round 1**: Each participating validator generates commitments (D matrix + MACs) using their Corona key share. Broadcast `Round1Data` to all other signers.
...
5. Corona adds approximately 200-300ms for quantum finality.

**ML-DSA Path (quantum-safe, identity proof):**
1. Each validator signs the finality message with their ML-DSA-65 key (FIPS 204).
2. ML-DSA signatures provide per-validator post-quantum identity binding.
3. Aggregated alongside Corona into the PQ proof field of QuasarCert.

**Block is final ONLY when all three certificates are valid:**
```go
type CertBundle struct {
    BLSAgg []byte  // 96B BLS aggregate signature
    PQCert []byte  // Aggregated Corona + ML-DSA proof
}
...
- The bundle is signed asynchronously with Corona threshold (0.6s online signing, 2.5s total including offline prep).
...
- Corona keys rotate when the validator set changes, rate-limited to at most 1 rotation per 10 minutes.
...
2. Each group generates its own Corona keys (2-of-3 threshold within group).
...
| **Q-day (BLS broken)** | Attacker can forge BLS but NOT Corona or ML-DSA lattice signatures. Block fails quantum check. Consensus halts rather than accepting unsafe fork. |
| **Post-quantum** | Security rests on lattice SVP hardness (2^160 operations). Both Corona threshold and ML-DSA identity signatures remain secure. |
| **Attack window** |  &lt;= PQ round time ( &lt;=50ms on mainnet). |

All three proofs (BLS, Corona, ML-DSA) are **REQUIRED** for Q-Chain validators in triple mode. `IsTripleMode()` returns true when all three are configured. The `Verify` method enforces strict triple-proof validation.
...
| Validator-PQ | Corona (Ring-LWE) | Post-quantum threshold shares | `$HOME/.lux/rt.key` |
...
- **Library**: `github.com/luxfi/corona` v0.2.0
...
| `PQSignatureTypeCorona` | Ring-LWE threshold | Quasar consensus threshold signatures |
| `PQSignatureTypeHybrid` | Classical + PQ combined | Full Quasar triple proofs (BLS + Corona + ML-DSA) |
...
| `MinEpochDuration` | 10 min | Minimum time between Corona key rotations |
...
| `DefaultGroupSize` | 3 | Validators per Corona group |
...
| C-Chain | Every block has CertBundle | Invalid without both BLS + Corona certificates |
| X-Chain | Vertex metadata references Q-block | Epoch sealed by quantum certificate |
| P-Chain | Validator set changes trigger epoch rotation | BLS and Corona keys synchronized |
...
consensus/
  protocol/
    ...
    quasar/         Post-quantum finality engine
      quasar.go     Signer (BLS+Corona+ML-DSA triple threshold)
      core.go       Quasar core (multi-chain block aggregator)
      engine.go     Engine interface implementation
      epoch.go      Epoch-based Corona key management
...
node/consensus/
  ...
  quasar/           Node-level Quasar integration
    quasar.go       Binds P-Chain + Q-Chain into triple finality
...
| `github.com/luxfi/corona` | v0.2.0 | Ring-LWE post-quantum threshold signatures |
...
5. **Finality proof**: Signed with Quasar dual signatures (BLS + Corona).
...
| Quantum adversary (BLS breakable) | Safe | Corona + ML-DSA lattice certificates prevent acceptance |
...
- Spend verifies single Corona signature (~1.8KB)
...
### Node: Quasar with Triple Finality
...
| Certificate validation fail | Key mismatch | Verify BLS and Corona keys registered for current epoch |
| Phase II timeout | Corona computation slow | Check hardware; consider grouped signing for large sets |
