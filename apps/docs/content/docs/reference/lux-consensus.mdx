...
|--------|------|----------|---------|--------|
| **Quasar** | Triple PQ | `protocol/quasar/` + `node/consensus/quasar/` | Primary consensus for all chains. Triple BLS+Pulsar+ML-DSA signatures run in parallel via `TripleSignRound1`. 2-round finality. | Production |
...
Quasar achieves triple quantum finality through parallel execution of classical and post-quantum signature paths (BLS + Pulsar + ML-DSA). The full algorithm:
...
3. Aggregated alongside Pulsar into the PQ proof field of QuasarCert.

**Block is final ONLY when all three certificates are valid:**
```go
type CertBundle struct {
    BLSAgg []byte  // 96B BLS aggregate signature
    PQCert []byte  // Aggregated Pulsar + ML-DSA proof
}
...
All three proofs (BLS, Pulsar, ML-DSA) are **REQUIRED** for Q-Chain validators in triple mode. `IsTripleMode()` returns true when all three are configured. The `Verify` method enforces strict triple-proof validation.
...
| `PQSignatureTypeHybrid` | Classical + PQ combined | Full Quasar triple proofs (BLS + Pulsar + ML-DSA) |
...
consensus/
  protocol/
    ...
    quasar/         Post-quantum finality engine
      quasar.go     Signer (BLS+Pulsar+ML-DSA triple threshold)
