...
|--------|------|----------|---------|--------|
| **Quasar** | Triple PQ | `protocol/quasar/` + `node/consensus/quasar/` | Primary consensus for all chains. Triple BLS+Ringtail+ML-DSA signatures run in parallel via `TripleSignRound1`. 2-round finality. | Production |
...
Quasar achieves triple quantum finality through parallel execution of classical and post-quantum signature paths (BLS + Ringtail + ML-DSA). The full algorithm:
...
3. Aggregated alongside Ringtail into the PQ proof field of QuasarCert.

**Block is final ONLY when all three certificates are valid:**
```go
type CertBundle struct {
    BLSAgg []byte  // 96B BLS aggregate signature
    PQCert []byte  // Aggregated Ringtail + ML-DSA proof
}
...
| **Q-day (BLS broken)** | Attacker can forge BLS but NOT Ringtail or ML-DSA lattice signatures. Block fails quantum check. Consensus halts rather than accepting unsafe fork. |
| **Post-quantum** | Security rests on lattice SVP hardness (2^160 operations). Both Ringtail threshold and ML-DSA identity signatures remain secure. |
| **Attack window** |  &lt;= PQ round time ( &lt;=50ms on mainnet). |

All three proofs (BLS, Ringtail, ML-DSA) are **REQUIRED** for Q-Chain validators in triple mode. `IsTripleMode()` returns true when all three are configured. The `Verify` method enforces strict triple-proof validation.
...
| `PQSignatureTypeHybrid` | Classical + PQ combined | Full Quasar triple proofs (BLS + Ringtail + ML-DSA) |
...
consensus/
  protocol/
    ...
    quasar/         Post-quantum finality engine
      quasar.go     Signer (BLS+Ringtail+ML-DSA triple threshold)
...
5. **Finality proof**: Signed with Quasar dual signatures (BLS + Ringtail).
...
| Quantum adversary (BLS breakable) | Safe | Ringtail + ML-DSA lattice certificates prevent acceptance |
