...
Lux Standard is the canonical contract library for building on Lux C-Chain and L2 chains (Zoo, Hanzo/AI, SPC, Pars). It provides Lux-native naming (LRC20 instead of ERC20), native bridge token support (LRC20B), post-quantum signer modules for Safe wallets, FHE-encrypted token balances and governance via T-Chain, and unified deployment scripts for all Lux ecosystem chains. Every DeFi primitive needed to build on Lux is here in one audited monorepo.
...
- Building any token (fungible, NFT, multi-token) on Lux C-Chain or L2 chains
