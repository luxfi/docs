...
Lux EVM (`github.com/luxfi/evm`) is the unified Go-based EVM execution engine for the Lux Network. It powers both the C-Chain (the primary EVM-compatible chain on the Lux primary network) and all sovereign L1 EVM chains. The former `coreth` codebase was **merged into this repo** -- `github.com/luxfi/coreth` is deprecated and no longer maintained separately.
...
- Deploying or interacting with smart contracts on C-Chain or any Lux EVM chain
...
- Running a custom sovereign L1 EVM with configurable fees, gas limits, and precompiles
...
| Module | `github.com/luxfi/evm` |
| Go version | 1.26.1 |
| C-Chain RPC | `http://<node>:9630/v1/chain/c` |
| C-Chain WebSocket | `ws://<node>:9630/v1/chain/c/ws` |
...
- **Activate-all-implicitly**: every Lux feature is live at genesis. Ethereum hardfork gates (Shanghai/Cancun) still control opcode behavior; Lux network features are not separately gated.
...
### Sovereign L1 EVM Mode

When deployed as a sovereign L1 chain:
...
- **Independent validator set** per chain
...
client, err := ethclient.Dial("http://localhost:9630/v1/chain/c")
if err != nil {
    return err
}
...
│       ├── network_upgrades.go -- Legacy header-schema timestamps (all set to genesis on Lux)
...
Lux runs the full feature set at genesis. The Ethereum hardfork gates
that control EVM opcode behavior (Shanghai, Cancun) stay — they are
bytecode-level rules. Lux-specific network features have no separate
timestamp gate.

| EVM gate | Effect |
|----------|--------|
| **Shanghai** | EIP-3651/3855/3860/6049 (EIP-4895 excluded — Lux has no PoS withdrawals) |
| **Cancun** | EIP-4844 blob schedule configuration required |

The legacy `params/extras/network_upgrades.go` timestamp fields exist
for header-byte compatibility with migrated chain-EVM state (LP-3641).
On Lux all of these fields are set to genesis (`InitiallyActiveTime`)
so every block sees the full ruleset.
...
| `0x0200...000B` | `corona` | Ring-LWE threshold lattice (post-quantum) |
| `0x012203` | `slhdsa` | SLH-DSA stateless hash signatures (FIPS 205) |
...
```
POST /v1/chain/c          -- Standard EVM JSON-RPC
WS   /v1/chain/c/ws           -- WebSocket subscriptions
POST /v1/chain/c/lux          -- Lux-specific APIs (validators)
POST /v1/chain/c/admin        -- Admin APIs (import/export, profiling)
```
