...
Lux EVM (`github.com/luxfi/evm`) is the unified Go-based EVM execution engine for the Lux Network. It powers both the C-Chain (the primary EVM-compatible chain on the Lux primary network) and all Chain/L1/L2 EVM chains. The former `coreth` codebase was **merged into this repo** -- `github.com/luxfi/coreth` is deprecated and no longer maintained separately.
...
- Running a custom L2 EVM with configurable fees, gas limits, and precompiles
...
### L2 EVM Mode (L1/L2 Chains)

When deployed as a chain/L1/L2 chain:
