...
description: "The coprocessor is a Go service with three binaries: gateway blockchain event listener, scheduler worker coordination, and tfhe-worker FHE com..."
---
...
1. Go 1.26.1
2. `github.com/luxfi/fhe` v1.7.5 -- the TFHE library. Lux-native FHE; the coprocessor uses it directly with no external library wrapping.
...
- `lux/lux-fhevm.md` -- FHEVM specification (Go primary, with C++ / GPU (Metal+CUDA) / FPGA accel paths)
