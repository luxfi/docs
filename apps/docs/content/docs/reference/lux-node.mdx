...
- **Serialization**: ZAP zero-copy wire (`github.com/luxfi/zap`) — every transaction, block, and VM↔node message is the buffer itself (struct = wire, no reflection codec). Protocol Buffers retained only for build-time codegen tooling.
