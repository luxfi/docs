...
Lux Node (`luxd`) is the core validator node for the Lux Network -- a multi-chain, multi-consensus blockchain with post-quantum dual finality (Quasar), 14 chain types, custom VM support, and W3C DID-based validator identity. Written in Go. The binary name is `luxd`, the module path is `github.com/luxfi/node`.

### Tech Stack

- **Language**: Go 1.26.1 (CGO enabled by default)
- **Consensus**: Quasar dual finality (BLS + Pulsar post-quantum threshold)
...
- **Serialization**: ZAP zero-copy wire (`github.com/luxfi/zap`) — every transaction, block, and VM↔node message is the buffer itself (struct = wire, no reflection codec, no protobuf). The `.proto` codegen path is fully retired — hand-written ZAP schemas replace it.
...
| `github.com/luxfi/codec` | v1.1.5 | Byte-packer primitives (`codec/wrappers`) for peer-handshake framing + Warp message codec — NOT the tx/block wire, which is native ZAP |
...
| `github.com/luxfi/corona` | v0.2.0 | Post-quantum Pulsar threshold signatures |
...
- Developing sovereign L1/L3 chains on the Lux network
...
2. **NEVER** use well-known test keys -- generate fresh keys for every deployment
...
```bash
# Node info
curl -X POST http://localhost:9630/v1/info \
  -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","id":1,"method":"info.getNodeID"}'

# Health check
curl -X POST http://localhost:9630/v1/health \
...
curl -X POST http://localhost:9630/v1/chain/p \
  -d '{"jsonrpc":"2.0","id":1,"method":"platform.getCurrentValidators","params":{}}'

# C-Chain: EVM JSON-RPC
curl -X POST http://localhost:9630/v1/chain/c \
  -d '{"jsonrpc":"2.0","id":1,"method":"eth_blockNumber","params":[]}'

# X-Chain: asset balance
curl -X POST http://localhost:9630/v1/chain/x \
...
│   ├── quasar/       — Dual quantum-safe finality (BLS + Pulsar)
...
├── pubsub/           — Pub/sub event system (durable messaging via hanzoai/pubsub)
...
|-------|--------|----|-----------|---------|
| **P-Chain** | P | platformvm | Linear (Nova) | Validator management, staking, chain/L1 creation |
...
|-------|--------|----|---------|
| **Q-Chain** | Q | quantumvm | Post-quantum cryptography, Pulsar keys, QuasarCert finality certificates |
...
1. **Linear (Nova)**: For linear chains (P-Chain, C-Chain, Q-Chain, etc.). Blocks form a single chain. Uses `ChainVM` interface. Wrapped by **ProposerVM (Linear++)** for congestion control via soft leader election.

2. **DAG (Nebula)**: For directed acyclic graph chains (X-Chain). Vertices can have multiple parents. Uses `DAGVertex` interface. Enables higher throughput via parallel transaction processing.

3. **Quasar**: The dual quantum-safe finality engine. Runs on top of linear consensus. Produces **dual finality proofs** (QuasarCert) requiring both:
...
SignatureTypeQuasar   = 2  // Dual BLS + Pulsar
...
- **ML-DSA Keys**: Post-quantum key pairs for dual staking (`staking/pq.go`, uses `github.com/luxfi/crypto/mldsa`)
...
| Quasar finality fails | Pulsar not initialized | Ensure all validators have both BLS and Pulsar keypairs; check `ErrCoronaNotConnected` |
