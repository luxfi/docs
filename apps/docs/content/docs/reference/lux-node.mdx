...
- **Consensus**: Quasar dual finality (BLS + Corona post-quantum threshold)
- **Crypto**: BLS aggregate signatures, ML-DSA-65 (post-quantum), ML-KEM, Corona threshold, Falcon, secp256k1
...
| `github.com/luxfi/corona` | v0.2.0 | Post-quantum Corona threshold signatures |
...
│   ├── quasar/       — Dual quantum-safe finality (BLS + Corona)
...
│   ├── quantumvm/    — Q-Chain (post-quantum crypto, Corona keys)
...
|-------|--------|----|---------|
| **Q-Chain** | Q | quantumvm | Post-quantum cryptography, Corona keys, QuasarCert finality certificates |
...
SignatureTypeQuasar   = 2  // Dual BLS + Corona
...
- **Corona Keys**: Post-quantum threshold signature shares
...
| Quasar finality fails | Corona not initialized | Ensure all validators have both BLS and Corona keypairs; check `ErrCoronaNotConnected` |
