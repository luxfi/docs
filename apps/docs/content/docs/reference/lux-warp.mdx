...
Lux Warp (`github.com/luxfi/warp`) is the **cross-chain messaging (XCM) protocol** for Lux Network. It defines a message format and cryptographic standard for secure inter-blockchain communication using BLS signature aggregation. V2 adds post-quantum safety via corona validation and optional private messaging via Z-chain FHE.
...
| `RegisterL1Validator` | Add validator to chain |
| `ChainToL1Conversion` | Chain conversion message |
...
1. **Post-Quantum Safety**: Random corona validation resists quantum attacks
