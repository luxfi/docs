...
Not just a test runner -- Netrunner orchestrates full L1/L3 stacks with cross-chain bridging, multi-network management with shared BadgerDB for cross-chain ACID transactions, and YAML-based stack manifests for reproducible environments.
...
| `AddPermissionlessValidator` | `POST /v1/control/addpermissionlessvalidator` | Add a validator to an elastic network |
| `RemoveChainValidator` | `POST /v1/control/removechainvalidator` | Remove a validator (legacy — superseded by `AddValidator`-based flow) |
...
| Geth | `geth` | Lux Geth execution client |
