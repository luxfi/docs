...
2. `getOAuthAuthorizeUrl()` constructs URL:
   ```
   https://lux.id/v1/iam/oauth/authorize?client_id=...&response_type=code&redirect_uri=.../callback&scope=openid+profile+email&state=<uuid>
   ```
...
| `NEXT_PUBLIC_IAM_AUTHORIZE_URL` | `https://lux.id/v1/iam/oauth/authorize` | OAuth2 authorize endpoint |
| `NEXT_PUBLIC_IAM_TOKEN_URL` | `https://lux.id/v1/iam/oauth/token` | OAuth2 token endpoint |
