...
#### EVM Chains
Uses `github.com/luxfi/indexer/evm` -- full EVM block/transaction indexing with `/v1/explorer/*` API.

| Chain | Port | Database | Description |
|-------|------|----------|-------------|
| C-Chain (Go) | 4000 | explorer_cchain | Smart contracts, full EVM indexing |
...
All indexers expose `/v1/explorer/*` endpoints:

**DAG chains**: `/v1/explorer/stats`, `/v1/explorer/vertices`, `/v1/explorer/vertices/:id`, `/v1/explorer/edges`, `WS /v1/explorer/dag/subscribe`

**Linear chains**: `/v1/explorer/stats`, `/v1/explorer/blocks`, `/v1/explorer/blocks/:id`, `/v1/explorer/blocks/height/:height`, `WS /v1/explorer/blocks/subscribe`

**EVM chains**: Full explorer API (blocks, transactions, addresses, tokens, stats, search).
...
| Package | Purpose |
|---------|---------|
| `evm/api/` | REST API server (`/v1/explorer/*`) |
...
```bash
# Single native chain
./bin/indexer -chain xchain -rpc http://localhost:9630/v1/chain/x -port 4200
