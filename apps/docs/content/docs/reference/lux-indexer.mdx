...
A single-process server that indexes all 5 Lux mainnet EVM L2 chains concurrently, sharing one PostgreSQL database with table prefixes. Configured via `configs/evmchains.yaml`.
