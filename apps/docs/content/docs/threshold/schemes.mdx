...
description: CGGMP21, FROST, BLS, and Pulsar threshold signature scheme comparison
icon: Layers
---
...
const (
    ...
    SchemeCorona SchemeID = 4  // Lattice-based (post-quantum)
)
```

## Comparison

| Property | CGGMP21 | FROST | BLS | Pulsar |
|----------|---------|-------|-----|----------|
...
| Implementation | `mpc/pkg/threshold` | `mpc/pkg/threshold` | `crypto/threshold/bls` | `Corona/threshold` |
...
Lattice-based threshold signatures using Module-LWE. The only post-quantum scheme. 2-round signing protocol with trusted dealer key generation. Implemented natively in the `Corona` repository.
...
Pulsar is used alongside BLS in Lux Quasar consensus for post-quantum security bundles, not as a standalone chain signing scheme.
