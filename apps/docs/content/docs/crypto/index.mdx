...
Lux consensus uses a hybrid BLS + Corona signing scheme:

```
Classical Layer:   BLS12-381 (aggregatable, 1-round)
                          +
Post-Quantum Layer: Corona (lattice threshold, 2-round)
```

| Package | Role | Notes |
|---------|------|-------|
| `signer/` | Hybrid BLS + Corona | Production consensus signing |
