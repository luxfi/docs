...
Lux consensus uses a hybrid BLS + Pulsar signing scheme:

```
Classical Layer:   BLS12-381 (aggregatable, 1-round)
                          +
Post-Quantum Layer: Pulsar (lattice threshold, 2-round)
```

| Package | Role | Notes |
|---------|------|-------|
| `signer/` | Hybrid BLS + Pulsar | Production consensus signing |
