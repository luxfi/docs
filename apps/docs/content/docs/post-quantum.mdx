...
- **Use in Lux**: TLS key exchange via X25519MLKEM768 hybrid, inter-node communication
...
Lux's Quasar consensus uses a **triple hybrid signature scheme** combining three independent cryptographic families:

| Scheme | Family | Purpose |
|--------|--------|---------|
| BLS (BLS12-381) | Pairing-based | Aggregatable validator signatures for efficient block finalization |
| Ringtail | Lattice-based | Post-quantum ring signatures for anonymous validator voting |
| ML-DSA-65 | Lattice-based (FIPS 204) | Post-quantum identity binding for validator authentication |

An attacker must break all three schemes simultaneously to forge a consensus vote. BLS provides efficient aggregation (reducing bandwidth), Ringtail provides voter privacy (preventing targeted attacks on specific validators), and ML-DSA-65 provides quantum-resistant identity authentication.
