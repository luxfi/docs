...
| Corona | Lattice-based | Post-quantum ring signatures for anonymous validator voting |
| ML-DSA-65 | Lattice-based (FIPS 204) | Post-quantum identity binding for validator authentication |

An attacker must break all three schemes simultaneously to forge a consensus vote. BLS provides efficient aggregation (reducing bandwidth), Corona provides voter privacy (preventing targeted attacks on specific validators), and ML-DSA-65 provides quantum-resistant identity authentication.
...
| `Corona Verify` | `0x0b16` | Verify Corona ring signatures in Solidity |
