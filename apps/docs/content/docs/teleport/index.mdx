...
Lux Teleport (`github.com/luxfi/teleport`) enables cross-chain token transfers between Lux chains and external chains. Tokens are burned on the source chain and minted on the destination chain, with MPC oracle nodes providing signature verification.
