...
post-quantum proof paths (Corona threshold + ML-DSA identity), so a
...
| **Quasar** | Threshold signing + epoch management (BLS + Corona + ML-DSA) |
...
type QuasarCert struct {
    ...
    Corona   []byte    // Pulsar/Ring-LWE 2-round threshold proof (~33 KB PQ)
...
The Corona layer is implemented by **Pulsar**
