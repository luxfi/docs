...
./indexer -chain cchain \
  -rpc http://localhost:9650/v1/chain/c \
  -port 4000

# Index X-Chain
./indexer -chain xchain \
  -rpc http://localhost:9650/v1/chain/x \
  -port 4200

# Index P-Chain
./indexer -chain pchain \
  -rpc http://localhost:9650/v1/chain/p \
  -port 4100
```
...
```bash
docker run -it \
  -e RPC_ENDPOINT=http://host.docker.internal:9650/v1/chain/c \
