...
```bash
curl -X POST https://kms.lux.cloud/api/v1/dynamic-secrets/postgres \
...
```bash
curl -X POST https://kms.lux.cloud/api/v1/dynamic-secrets/postgres/app-db/lease \
  -H "Authorization: Bearer $KMS_TOKEN" \
  -d '{"ttl": "2h"}'
```
...
```bash
curl -X DELETE https://kms.lux.cloud/api/v1/dynamic-secrets/leases/lease-abc123 \
  -H "Authorization: Bearer $KMS_TOKEN"
```
