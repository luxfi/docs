...
```bash
curl -X POST https://kms.lux.cloud/api/v1/kms/keys/my-key/encrypt \
...
```bash
curl -X POST https://kms.lux.cloud/api/v1/kms/keys/my-key/decrypt \
  -H "Authorization: Bearer $KMS_TOKEN" \
  -d '{
    "ciphertext": "vault:v1:AbCdEf..."
  }'
...
```bash
curl -X POST https://kms.lux.cloud/api/v1/kms/keys/signing-key/sign \
  -H "Authorization: Bearer $KMS_TOKEN" \
  -d '{
    "input": "aGVsbG8=",
...
```bash
# Rotate key
curl -X POST https://kms.lux.cloud/api/v1/kms/keys/my-key/rotate \
  -H "Authorization: Bearer $KMS_TOKEN"

# Rewrap old ciphertext with new key version
curl -X POST https://kms.lux.cloud/api/v1/kms/keys/my-key/rewrap \
  -H "Authorization: Bearer $KMS_TOKEN" \
...
```bash
curl -X POST https://kms.lux.cloud/api/v1/kms/keys/my-key/generate-data-key \
  -H "Authorization: Bearer $KMS_TOKEN"
```
