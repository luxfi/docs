---
title: Platform API
description: P-Chain Platform API reference for staking, validators, and chains
---

# Platform API

The Platform API provides access to the P-Chain, which manages validators, staking, and chains. All Platform API methods are accessed at the `/ext/P` endpoint (or equivalently `/ext/bc/P`).
...
|------|------|-------------|
| `subnetID` | string | Chain ID (omit for Primary Network) |
...
| Name | Type | Description |
|------|------|-------------|
| `controlKeys` | []string | Addresses that can add validators to the chain |
...
The `txID` is also the new chain's ID.

### platform.getSubnets

Lists all chains or a specific chain.
...
Adds a validator to a chain. The validator must already be a Primary Network validator.
...
- [Run a Validator](/nodes/validator) - Set up and manage a validator
- [Chains](/cloud/chains) - Create and configure chains
- [Tokenomics](/tokenomics) - Staking economics
