...
func main() {
    ...
    // Testnet
    // c := client.New("https://api.lux-test.network", 443)
...
```go
nets, err := platform.GetNets(ctx, nil)
if err != nil {
    log.Fatal(err)
}

for _, net := range nets {
    fmt.Printf("Network: %s (control keys: %d, threshold: %d)\n",
        net.ID, len(net.ControlKeys), net.Threshold)
}
```

## EVM (C-Chain) Client

For C-Chain interactions, use the standard Lux Geth compatible client from the SDK.
...
- [JavaScript SDK Reference](/api-reference/sdk-js) - JS/TS SDK for web applications
- [Platform API](/api-reference/platform) - Raw P-Chain API methods
- [JSON-RPC API](/api-reference/json-rpc) - C-Chain JSON-RPC methods
- [Smart Contracts](/cloud/contracts) - Deploy and interact with contracts
