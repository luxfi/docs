...
The Info API provides information about the node's identity, version, network status, and peer connections. All methods are available at the `/v1/info` endpoint.

## Endpoint

```
POST https://api.lux.network/v1/info
POST http://localhost:9650/v1/info
```
...
- [Health API](/api-reference/health) - Node health checks
- [Full Node Guide](/nodes/full-node) - Node setup and configuration
- [Validator Guide](/nodes/validator) - Validator management
