...
|----------|--------|-------------|
| `/v1/health` | GET or POST | Full health check with details |
| `/v1/health/liveness` | GET | Simple liveness probe (200 = alive) |
| `/v1/health/readiness` | GET | Readiness probe (200 = ready to serve) |
...
```bash
curl -s http://localhost:9650/v1/health | jq .
```
...
}' -H 'content-type:application/json' http://localhost:9650/v1/health
```
...
```bash
curl -s -o /dev/null -w "%{http_code}" http://localhost:9650/v1/health/liveness
...
```bash
curl -s -o /dev/null -w "%{http_code}" http://localhost:9650/v1/health/readiness
...
The health status is also available as a Prometheus metric at `/v1/metrics`:
...
```bash
#!/bin/bash
STATUS=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:9650/v1/health)
...
- [Info API](/api-reference/info) - Node identity and version information
- [Full Node Guide](/nodes/full-node) - Node setup and monitoring
- [Validator Guide](/nodes/validator) - Uptime monitoring for validators
