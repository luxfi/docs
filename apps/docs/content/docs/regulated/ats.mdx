...
// Install FinCEN-mandated baseline rules (in-process)
monitoringSvc := aml.NewMonitoringService()
aml.InstallDefaultRules(monitoringSvc)

// Optional: delegate real-time scored monitoring to the Lux AML engine
// (luxfi/aml, amld). Point AML_ENGINE_URL at the engine and screen each
// transaction via POST /v1/aml/transactions. See /compliance/aml.
amlEngineURL := os.Getenv("AML_ENGINE_URL") // e.g. http://amld.svc.cluster.local:8090
```
...
func main() {
    ...
    // 3. Compliance — in-process baseline rules
    monSvc := aml.NewMonitoringService()
    aml.InstallDefaultRules(monSvc)

    // Optional: delegate real-time scored monitoring to the Lux AML engine
    // (luxfi/aml, amld). Screen transactions via POST /v1/aml/transactions.
    amlEngineURL := os.Getenv("AML_ENGINE_URL") // e.g. http://amld.svc.cluster.local:8090
    _ = amlEngineURL // see /compliance/aml
...
| `AML_ENGINE_URL` | No | [Lux AML engine](/compliance/aml) endpoint for real-time scored monitoring (`amld`, `/v1/aml/*`) |
...
See `<tenant>/ats` on GitHub (`https://github.com/<tenant>/ats`) for a working ATS implementation built on these components.
