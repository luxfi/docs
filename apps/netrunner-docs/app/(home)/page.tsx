    ...
    <main className="flex flex-1 flex-col items-center justify-center text-center px-4 py-16">
      <div className="max-w-3xl mx-auto">
        <div className="inline-flex items-center gap-2 rounded-full border border-fd-border px-4 py-1.5 text-sm text-fd-muted-foreground mb-6">
          Simulation · Benchmarks · Testing · CI/CD
        </div>
        <h1 className="text-5xl md:text-7xl font-bold mb-6 bg-gradient-to-r from-teal-400 to-emerald-400 bg-clip-text text-transparent">
          Lux Netrunner
        </h1>
        <p className="text-lg text-fd-muted-foreground mb-8 max-w-2xl mx-auto">
          Network testing and simulation tool.
          Spin up multi-node networks, benchmark performance, and verify consensus.
        </p>
        <div className="rounded-lg border border-fd-border bg-fd-card p-4 mb-8 max-w-md mx-auto font-mono text-sm">
          <span className="text-fd-muted-foreground">$</span>{' '}go get github.com/luxfi/netrunner@latest
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-left">
          {[
            { name: 'Network Simulation', desc: 'Multi-node networks with custom topologies' },
            { name: 'Performance Testing', desc: 'TPS benchmarks and latency measurement' },
            { name: 'Consensus Verification', desc: 'Verify consensus correctness under load' },
            { name: 'Partition Testing', desc: 'Simulate network partitions and failures' },
            { name: 'Custom Topology', desc: 'Configure node count, connections, and delays' },
            { name: 'CI/CD Integration', desc: 'Automated testing in GitHub Actions and more' },
          ].map((c) => (
