...
description: Este tutorial demuestra cómo implementar una Subred con permisos en la Testnet.
sidebar_label: En la Testnet
pagination_label: Implementar una Subred con permisos en la Testnet
sidebar_position: 1
---

# Implementar una Subred con permisos en la Testnet

:::note

Este documento describe cómo usar Lux-CLI para implementar una Subred en `Testnet`.

:::

Después de probar una Subred en un equipo local siguiendo [este tutorial](/build/subnet/deploy/local-subnet.md),
el siguiente paso es probarlo en la Testnet `Testnet`.

En este artículo, se muestra cómo hacer lo siguiente en la Testnet `Testnet`.
...
- 1+ nodos en ejecución y completamente inicializados en la Testnet `Testnet`. Echa un vistazo a la sección
...
[Subnet-EVM](https://github.com/luxfi/subnet-evm) es la VM que define las Cadenas de Contratos de la Subred.
Subnet-EVM es una versión simplificada de [Cadena C de Lux](https://github.com/luxfi/coreth).
...
[Ejecutar un Nodo Lux Manualmente](/nodes/run/node-manually.md#connect-to-testnet)
y este paso a continuación en particular para iniciar tu nodo en `Testnet`:

_Para conectarte a la Testnet en lugar de a la red principal, usa el argumento `--network-id=Testnet`_

También vale la pena señalar que
[solo se necesita 1 LUX para convertirse en un validador en la Testnet](/nodes/validate/what-is-staking.md#testnet)
y puedes obtener el token de prueba desde el [faucet](https://faucet.lux.network/).

Para obtener el NodeID de este nodo `Testnet`, llama al siguiente comando curl a [info.getNodeID](/reference/luxd/info-api.md#infogetnodeid):
...
Con más datos en `Testnet`, puede llevar un tiempo inicializar la Testnet `Testnet` desde cero.
...
ya que las operaciones en `Mainnet` requieren el uso de ledger, mientras que en `Testnet` es opcional.
...
Solo debes usar la clave privada creada para este tutorial para operaciones de prueba en `Testnet` u
...
tanto para la red local predeterminada como para `Testnet`. Esta última
(`P-testnet1a3azftqvygc4tlqsdvd82wks2u7nx85rhk6zqh`) es la que se necesita para este tutorial.
...
Realiza estos pasos solo para seguir este tutorial en direcciones `Testnet`. Para acceder a la billetera de `Mainnet`,
...
Después de seguir estos 3 pasos, tu clave de prueba debería tener ahora un saldo en la P-Chain en `Testnet` Testnet.

## Crear una Subred EVM

Crear una Subred con `Lux-CLI` para `Testnet` funciona de la misma manera que con una
...
comando `create`, luego desplegando primero en una red local y sucesivamente en `Testnet` - y
...
+----------------------------+----------------------------------------------------+
| SubnetID de Testnet           | XTK7AM2Pw5A4cCtQ3rTugqbeLCU9mVixML3YwwLYUJ4WXN2Kt  |
+----------------------------+----------------------------------------------------+
| BlockchainID de Testnet       | 5ce2WhnyeMELzg9UtfpCDGNwRa2AzMzRhBWfTqmFuiXPWE4TR  |
+----------------------------+----------------------------------------------------+
...
Este tutorial trata sobre el despliegue en `Testnet`, así que navega con las teclas de flecha hasta `Testnet` y presiona enter.
...
Además, este tutorial asume que un nodo está en funcionamiento, completamente bootstrapado en `Testnet`, y se ejecuta
desde la **misma** máquina.

```bash
✔ Testnet
Desplegando [testsubnet] en Testnet
Usa las teclas de flecha para navegar: ↓ ↑ → ←
...
Asegúrate de proporcionar direcciones de la **cadena P Testnet** -`P-Testnet....`-.
...
```bash
✔ Agregar clave de control
Ingresa la dirección de la cadena P (Ej: `P-...`): P-testnet1vaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaasz
Usa las teclas de flecha para navegar: ↓ ↑ → ←
? Establecer claves de control:
    Agregar clave de control
  ▸ Hecho
...
Bien hecho. Acabas de crear tu propia Subnet con tu propia Subnet-EVM en ejecución en `Testnet`.
...
```bash
✔ Ruta de tu directorio de plugins de luxd (probablemente luxd/build/plugins): /home/user/go/src/github.com/luxfi/luxd/build/plugins
```
...
```shell
✔ Ruta de tu directorio de plugins de luxd (probablemente luxd/build/plugins): /home/user/go/src/github.com/luxfi/luxd/build/plugins█
Binario de la VM escrito en /home/user/go/src/github.com/luxfi/luxd/build/plugins/tGBrMADESojmu5Et9CpbGCrmVf9fiAJtZM5ZJ3YVDj5JTu2qw
...
```bash
Usa las teclas de flecha para navegar: ↓ ↑ → ←
? Elige una red en la que desplegar. Este comando solo admite Testnet actualmente.:
  ▸ Testnet
    Mainnet
```
...
¡Esto significa que el nodo es ahora un validador en la Subnet dada en `Testnet`!
