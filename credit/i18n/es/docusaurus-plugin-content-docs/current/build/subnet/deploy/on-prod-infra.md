...
Las instrucciones generales sobre cómo usar un dispositivo Ledger con Lux se pueden encontrar aquí.
...
Si quieres inspeccionar el proceso de sincronización de la Subnet, puedes usar la llamada RPC para verificar el [estado de la cadena de bloques](/reference/luxd/p-chain/api.md#platformgetblockchainstatus).

Para obtener una visión más profunda del funcionamiento de la Subnet, consulta el registro de la cadena de bloques. Por defecto, el registro se puede encontrar en `~/.luxd/logs/ChainID.log` donde reemplazas `ChainID` por el ID real de la cadena de bloques en tu Subnet.
...
Si tienes alguna pregunta, duda o quieres chatear, por favor visita nuestro [servidor de Discord](https://chat.lux.network/), donde tenemos un canal dedicado `#subnet-chat` para hablar sobre todo lo relacionado con Subnets.
