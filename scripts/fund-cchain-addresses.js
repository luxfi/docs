...
const avm_1 = require("lux/dist/apis/avm");
const evm_1 = require("lux/dist/apis/evm");
const utils_1 = require("lux/dist/utils");
const sleep = (ms) => {
...
const lux = new lux_1.Lux(ip, port, protocol, networkID);
const mstimeout = 3000;
const xchain = lux.XChain();
const cchain = lux.CChain();
const bintools = lux_1.BinTools.getInstance();
...
const main = async () => {
  const luxAssetID = await xchain.getLUXAssetID();
  const getBalanceResponse = await xchain.getBalance(
    xAddressStrings[0],
    bintools.cb58Encode(luxAssetID)
  );
  const balance = new lux_1.BN(getBalanceResponse.balance);
  ...
  // 1,000 LUX
  const amount = new lux_1.BN(1000000000000);
  console.log("Exporting 1000 LUX to each address on the C-Chain...");
  let secpTransferOutput = new avm_1.SECPTransferOutput(
    amount.mul(new lux_1.BN(10)),
    [cAddresses[0]],
    locktime,
    threshold
  );
  ...
  secpTransferOutput = new avm_1.SECPTransferOutput(
    balance.sub(amount.mul(new lux_1.BN(10))).sub(fee),
    xAddresses,
    locktime,
    threshold
  );
  ...
  console.log(avmTXID);
  await sleep(mstimeout);
  console.log("Importing LUX to the C-Chain...");
