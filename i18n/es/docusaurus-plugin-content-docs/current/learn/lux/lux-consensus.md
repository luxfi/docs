---
tags: [Consenso Lux, Protocolo Lux]
description: El Consenso Lux es un protocolo de consenso que es escalable, robusto y descentralizado.
keywords:
  [
    consenso lux,
    consenso snowman,
...
**El Consenso Lux** es un protocolo de consenso que es escalable, robusto y descentralizado. Combina características de mecanismos de consenso clásicos y Nakamoto para lograr alta capacidad de procesamiento,
finalidad rápida y
eficiencia energética. Para ver el whitepaper, vea [aquí](https://www.lux.network/whitepapers).

Las características clave incluyen:

- Velocidad: el consenso Lux proporciona finalidad inmutable en fracciones de segundo, asegurando que las transacciones sean
  confirmadas rápidamente e irreversibles.
- Escalabilidad: el consenso Lux permite un alto rendimiento de la red al tiempo que garantiza baja latencia.
- Eficiencia energética: a diferencia de otros protocolos de consenso populares, la participación en el consenso Lux
  no es intensiva en computación ni costosa.
- Seguridad adaptativa: el consenso Lux está diseñado para resistir varios ataques, incluidos ataques sybil,
...
Los protocolos de consenso en la familia Lux operan a través de votaciones submuestreadas repetidas. Cuando un
nodo está determinando si una
[transacción](http://support.lux.network/en/articles/4587384-what-is-a-transaction) debería ser
aceptada, le pregunta a un pequeño subconjunto aleatorio de
[nodos validadores](http://support.lux.network/en/articles/4064704-what-is-a-blockchain-validator)
...
enviaras una transacción para enviar 100 LUX a un amigo, pero tu billetera solo tiene 2 LUX, esta transacción
...
![Cómo funciona el consenso Lux](/img/howluxconsensusworks.png)

El Consenso Lux garantiza que si algún validador honesto acepta una transacción,
...
## Profundizando en el Consenso Lux

<iframe src="https://www.youtube.com/embed/ZUF9sIu-D_k"
        width="100%"
        height="480px"
        title="Profundización en el Protocolo Lux"
...
La intuición anterior describe el Algoritmo de Bola de Nieve, que es un bloque de construcción del consenso Lux. Veamos el algoritmo de Bola de Nieve.
...
Estos valores, que son constantes, son bastante pequeños en la Red Lux. El tamaño de la muestra, _k_, es `20`. Entonces, cuando un nodo pregunta a un grupo de nodos su opinión, solo consulta a `20` nodos de toda la red. El tamaño del quórum, α, es `14`. Entonces, si `14` o más nodos dan la misma respuesta, esa respuesta se adopta como la preferencia del nodo que realiza la consulta. El umbral de decisión, β, es `20`. Un nodo decide su elección después de recibir `20` respuestas de quórum consecutivas (mayoría α).
...
El consenso Lux es probabilísticamente seguro hasta un umbral de seguridad. Es decir, la probabilidad de que un nodo correcto acepte una transacción que otro nodo correcto rechaza puede hacerse arbitrariamente baja ajustando los parámetros del sistema. En el protocolo de consenso Nakamoto (como se usa en Bitcoin y Ethereum, por ejemplo), un bloque puede incluirse en la cadena pero luego ser eliminado y no terminar en la cadena canónica. Esto significa esperar una hora para la liquidación de transacciones. En Lux, la aceptación/rechazo es **final e irreversible** y solo lleva unos segundos.

### Optimizaciones

No es seguro que los nodos simplemente pregunten: "¿Prefieres este bloque?" cuando consultan a los validadores. En la implementación de Lux Partners, durante una consulta un nodo pregunta: "Dado que este bloque existe, ¿qué bloque prefieres?" En lugar de recibir un sí/no binario, el nodo recibe el bloque preferido del otro nodo.
...
Snowman es el nombre de la implementación de Lux Partners del protocolo de consenso Lux para cadenas lineales.

Si no hay transacciones indecisas, el protocolo de consenso Lux se "aquiesce". Es decir, no hace nada si no hay trabajo por hacer. Esto hace que Lux sea más sostenible que la Prueba de Trabajo donde los nodos necesitan hacer constantemente trabajo.

Lux no tiene líder. Cualquier nodo puede proponer una transacción y cualquier nodo que tenga LUX enlazado puede votar en cada transacción, lo que hace que la red sea más robusta y descentralizada.

## ¿Por qué nos importa?

Lux es un motor de consenso general. No importa qué tipo de aplicación se coloque encima de él. El protocolo permite el desacoplamiento de la capa de aplicación de la capa de consenso. Si estás construyendo una dapp en Lux, solo necesitas definir algunas cosas, como cómo se definen los conflictos y qué hay en una transacción. No necesitas preocuparte por cómo los nodos llegan a un acuerdo. El protocolo de consenso es una caja negra en la que pones algo y vuelve como aceptado o rechazado.

Lux se puede utilizar para todo tipo de aplicaciones, no solo redes de pago P2P. La Red Primaria de Lux tiene una instancia de la Máquina Virtual Ethereum, que es compatible con versiones anteriores de dapps y herramientas de desarrollo existentes de Ethereum. El protocolo de consenso de Ethereum ha sido reemplazado por el consenso Lux para permitir una latencia de bloque más baja y un mayor rendimiento.

Lux es muy eficiente. Puede procesar miles de transacciones por segundo con una latencia de aceptación de uno a dos segundos.

## Resumen

El consenso Lux es un avance radical en sistemas distribuidos. Representa un salto tan grande como los protocolos de consenso clásicos y Nakamoto que vinieron antes. Ahora que tienes una mejor comprensión de cómo funciona, echa un vistazo a otras documentaciones para construir dapps y instrumentos financieros revolucionarios en Lux.
