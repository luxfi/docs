...
resource "google_compute_disk" "node_disk" {
    ...
    type  = "pd-ssd"
    zone    = "${var.zone}"
    #snapshot = "https://www.googleapis.com/compute/v1/projects/my-lux-project/global/snapshots/my-lux-snapshot" - if you have a snapshot you can leverage it here
    size = 400
    count = "${var.total_instances}"
}
...
#!/bin/bash
# add anything here that you want to execute when the machine starts... such as the lux installer shell script.
SCRIPT
  }
