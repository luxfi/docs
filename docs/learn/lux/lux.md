---
tags: [Lux, Tokenomics]
description: LUX is the limited-supply token of the Lux network, used to cover fees, enhance security via staking, and facilitate transactions across its various Subnets.
keywords: [documentation, lux, lux token, tokenomics, lux utility]
sidebar_label: LUX Token
---

# LUX

LUX is the native utility token of Lux. It’s a hard-capped, scarce asset that is used to
pay for fees, secure the platform through staking, and provide a basic unit of account between the
multiple Subnets created on Lux. 

:::info

- `1 nLUX` is equal to `0.000000001 LUX`.
...
Any entity trying to execute a transaction on Lux pays a corresponding fee (commonly known as
