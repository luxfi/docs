...
Lux can run multiple blockchains. Each blockchain is an instance of a
[Virtual Machine](/learn/lux/subnets-overview.md#virtual-machines), much like an object in
an object-oriented language is an instance of a class.
That's, the VM defines the behavior of the blockchain.

[Subnet-EVM](https://github.com/luxdefi/subnet-evm) is the VM that defines the Subnet Contract
Chains. Subnet-EVM is a simplified version of [Lux C-Chain](https://github.com/luxdefi/coreth).
...
1. A newly created key has no funds on it. Send funds via transfer to its correspondent addresses
   ...
   [`https://faucet.lux.network`](https://faucet.lux.network/) using your **C-Chain address**.

2. **Export** your key via the `lux key export` command. The output is your private key,
   which will help you [import](https://support.lux.network/en/articles/6821877-core-extension-how-can-i-import-an-account)
...
See [here](/nodes/validate/add-a-validator.md#add-a-validator-with-wallet) on how to
become a validator.
...
`/etc/luxd/config.json` - just make sure the tool has **write** access to the file. Or you
...
[transaction](/reference/luxd/p-chain/api.md#platformaddsubnetvalidator), you will need to specify
...
including the public API server `https://api.lux-test.network/`, to connect to Core.
