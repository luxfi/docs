...
Depending on the `networkID` which is passed in when instantiating `Lux` the encoded addresses will have a distinctive HRP per each network. LuxJS also has address encoding for past networks `cascade`, `denali`, and `everest`.
