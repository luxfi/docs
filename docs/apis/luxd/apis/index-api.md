...
There is a Go implementation of an Index API client. See documentation [here](https://pkg.go.dev/github.com/luxdefi/luxd/indexer#Client). This client can be used inside a Go program to connect to an LuxGo node that is running with the Index API enabled and make calls to the Index API.
