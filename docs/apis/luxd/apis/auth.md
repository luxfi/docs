...
This API is only reachable if the node is started with [config flag](../../../nodes/maintain/luxd-config-flags.md)`--api-auth-required`. If the node is started without this CLI, API calls do not require authorization tokens, so this API is not reachable. This API never requires an authorization token to be reached.
