---
description: More information available regarding Lux Go APIs and learning how to interact with the C-Chain.
sidebar_position: 4
---
...
:::info
Ethereum has its own notion of `networkID` and `chainID`. These have no relationship to Lux’s view of networkID and chainID and are purely internal to the [C-Chain](../../../overview/getting-started/lux-platform.md#contract-chain-c-chain). On Mainnet, the C-Chain uses `1` and `43114` for these values. On the Fuji Testnet, it uses `1` and `43113` for these values. `networkID` and `chainID` can also be obtained using the `net_version` and `eth_chainId` methods.

:::

## Deploying a Smart Contract

[Deploy a Smart Contract on Lux Using Remix and MetaMask](../../../dapps/smart-contracts/deploy-a-smart-contract-on-lux-using-remix-and-metamask.md)
...
Lux offers an API interface identical to Geth's API except that it only supports the following services:
...
In addition to the standard Ethereum APIs, Lux offers `eth_getAssetBalance` to retrieve the balance of first class Lux Native Tokens on the C-Chain (excluding LUX, which must be fetched with `eth_getBalance`).
...
To interact with the `lux` specific RPC calls on the C-Chain:

```sh
/ext/bc/C/lux
```

To interact with other instances of the EVM LUX endpoints:

```sh
/ext/bc/blockchainID/lux
```
...
Export an asset from the C-Chain to X-Chain or P-Chain. After calling this method, you must call the X-Chain's [`avm.import`](x-chain.md#avmimport) or P-Chain's [`platform.import`](p-chain.md#platformimportlux).
...
- `assetID` is the ID of the asset. To export LUX use `"LUX"` as the `assetID`.
...
**DEPRECATED—instead use** [**lux.export**](c-chain.md#luxexport).

Send LUX from the C-Chain to X-Chain or P-Chain. After calling this method, you must call the X-Chain's [`avm.import`](x-chain.md#avmimport) or P-Chain's [`platform.import`](p-chain.md#platformimportlux) with assetID `LUX` on the X-Chain to complete the transfer.
...
Get the private key that controls a given address. The returned private key can be added to a user with `lux.importKey`.
...
- `privateKey` is the CB58 endcoded string representation of the private key that controls `address`. It has a `PrivateKey-` prefix and can be used to import a key via `lux.importKey`.
...
Suppose we want all UTXOs that reference at least one of `C-lux18jma8ppw3nhx5r4ap8clazz0dps7rv5ukulre5`.
...
Finalize the transfer of a non-LUX or LUX from X-Chain or P-Chain to the C-Chain. Before this method is called, you must call the X-Chain’s [`avm.export`](x-chain.md#avmexport) or P-Chain’s [`platform.exportLUX`](p-chain.md#platformexportlux) with assetID `LUX` to initiate the transfer.
...
**DEPRECATED—instead use** [**lux.import**](c-chain.md#luximport)

Finalize a transfer of LUX from the X-Chain or P-Chain to the C-Chain. Before this method is called, you must call the X-Chain’s [`avm.export`](x-chain.md#avmexport) or P-Chain’s [`platform.exportLUX`](p-chain.md#platformexportlux) with assetID `LUX` to initiate the transfer.
...
- `to` is the address the LUX is sent to. It should be in hex format.
- `sourceChain` is the ID or alias of the chain the LUX is being imported from. To import funds from the X-Chain, use `"X"`; for the P-Chain, use `"P"`.
- `baseFee` is the base fee that should be used when creating the transaction. If omitted, a suggested fee will be used.
- `username` is the user that controls the address that transaction will be sent from.
- `password` is `username`‘s password.
...
```sh
lux.getAtomicTxStatus({txID: string}) -> {
  status: string,
