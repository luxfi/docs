...
A delegator stakes LUX and specifies a validator (the delegatee) to validate on their behalf. The delegatee has an increased probability of being sampled by other validators (weight) in proportion to the stake delegated to them.
...
Once you issue the transaction to add a node as a delegator, there is no way to change the parameters. **You can’t remove a stake early or change the stake amount, node ID, or reward address.** Please make sure you’re using the correct values. If you’re not sure, please reach out to us on [Discord.](https://chat.lux.network/)
...
- `endTime` is the Unix time when the delegator stops delegating (and staked LUX is returned).
- `stakeAmount` is the amount of nLUX the delegator is staking.
...
Add a validator to the Primary Network. You must stake LUX to do this. If the node is sufficiently correct and responsive while validating, you receive a reward when end of staking period is reached. The validator’s probability of being sampled by other validators during consensus is in proportion to the amount of LUX staked.
...
:::info
Note that once you issue the transaction to add a node as a validator, there is no way to change the parameters. **You can’t remove stake early or change the stake amount, node ID, or reward address.** Please make sure you’re using the correct values. If you’re not sure, please reach out to us on [Discord.](https://chat.lux.network/)
:::
...
- `endTime` is the Unix time when the validator stops validating the Primary Network (and staked LUX is returned).
- `stakeAmount` is the amount of nLUX the validator is staking.
...
Send LUX from an address on the P-Chain to an address on the X-Chain or C-Chain. After issuing this transaction, you must call the X-Chain's [`avm.import`](x-chain.md#avmimport) or C-Chain's [`lux.import`](c-chain.md#luximport) with assetID `LUX` to complete the transfer.
...
- `amount` is the amount of nLUX to send.
- `to` is the address on the X-Chain or C-Chain to send the LUX to.
- `from` are the addresses that you want to use for this operation. If omitted, uses any of your addresses as needed.
- `changeAddr` is the address any change will be sent to. If omitted, change is sent to one of the addresses controlled by the user.
- `username` is the user sending the LUX and paying the transaction fee.
- `password` is `username`‘s password.
...
- `balance` is the total balance, in nLUX.
- `unlocked` is the unlocked balance, in nLUX.
- `lockedStakeable` is the locked stakeable balance, in nLUX.
- `lockedNotStakeable` is the locked and not stakeable balance, in nLUX.
...
Returns an upper bound on the number of LUX that exist. This is an upper bound because it does not account for burnt tokens, including transaction fees.
...
- `supply` is an upper bound on the number of LUX that exist, denominated in nLUX.
...
- `validators`:
  ...
  - `stakeAmount` is the amount of nLUX this validator staked. Omitted if `subnetID` is not the Primary Network.
...
Get the minimum amount of LUX required to validate the Primary Network and the minimum amount of LUX that can be delegated.
...
- `delegators`:
  ...
  - `endTime` is the Unix time when the delegator stops.
  - `stakeAmount` is the amount of nLUX this delegator staked. Omitted if `subnetID` is not the Primary Network.
  - `nodeID` is the validating node’s node ID.
...
Get the amount of nLUX staked by a set of addresses. The amount returned does not include staking rewards.
...
- `staked` is the amount of nLUX staked by addresses provided.
...
Get the total amount of nLUX staked on the Primary Network.
...
Suppose we want all UTXOs that reference at least one of `P-lux18jma8ppw3nhx5r4ap8clazz0dps7rv5ukulre5` and `P-lux1d09qn852zcy03sfc9hay2llmn9hsgnw4tp3dv6`.
...
Complete a transfer of LUX from the X-Chain, or C-Chain to the P-Chain.

Before this method is called, you must call the X-Chain’s [`avm.export`](x-chain.md#avmexport) or C-Chain’s [`lux.export`](c-chain.md#luxexport) method with assetID `LUX` to initiate the transfer.
...
- `to` is the ID of the address the LUX is imported to. This must be the same as the `to` argument in the corresponding call to the X-Chain’s or C-Chain's `export`.
