...
One of the core features of Lux is the ability to create new, custom blockchains, which are defined by [Virtual Machines (VMs)](../subnets/README.md#virtual-machines)

In this tutorial, we’ll create a very simple VM. The blockchain defined by the VM is a [TimestampVM](https://github.com/luxdefi/timestampvm/tree/v1.2.1). Each block in the blockchain contains the timestamp when it was created along with a 32-byte piece of data (payload). Each block’s timestamp is after its parent’s timestamp.
...
You can see the full file from [here.](https://github.com/luxdefi/luxd/blob/v1.7.4/snow/engine/common/vm.go)
...
Let’s look at this interface and its methods. You can see the full file from [here.](https://github.com/luxdefi/luxd/blob/v1.7.4/snow/consensus/snowman/block.go)
...
This interface is the superset of every decidable object, such as transactions, blocks and vertices. You can see the full file from [here.](https://github.com/luxdefi/luxd/blob/v1.7.4/snow/choices/decidable.go)
...
Let’s write our VM, which implements `block.ChainVM` and whose blocks implement `snowman.Block`. You can also follow the code in the [TimestampVM repository](https://github.com/luxdefi/timestampvm/tree/v1.2.1).
...
This VM has a [build script](https://github.com/luxdefi/timestampvm/blob/v1.2.1/scripts/build.sh) that builds an executable of this VM (when invoked, it runs the `main` method from above.)
...
If no argument is given, the path defaults to a binary named with default VM ID: `$GOPATH/src/github.com/luxdefi/luxd/build/plugins/tGas3T58KzdjLHhBDMnH2TvrddhqTji5iZAMZ3RXs2NLpSnhH`
...
It's possible to give an alias for these IDs. For example, we can alias `TimestampVM` by creating a JSON file at `~/.luxd/configs/vms/aliases.json` with:
...
```bash
cp -n <path to your binary> $GOPATH/src/github.com/luxdefi/luxd/build/plugins/
```
