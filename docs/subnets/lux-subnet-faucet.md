...
- [Dexalot Testnet](https://faucet.lux.network?subnet=dexalot) and many more.

You can use this [repository](https://github.com/luxdefi/lux-faucet) to deploy your faucet or just make a PR with the [configurations](https://github.com/luxdefi/lux-faucet/blob/main/config.json) of the Subnet. This faucet comes with many features like multiple chain support, custom rate-limiting per Subnet, captcha verification, and concurrent transaction handling.
...
- Fork the [repository](https://github.com/luxdefi/lux-faucet) to deploy your faucet for any EVM chain

## Adding a New Subnet

You can also integrate a new Subnet on the live [faucet](https://faucet.lux.network) with just a few lines of configuration parameters. All you have to do is make a PR on the [Lux Faucet](https://github.com/luxdefi/lux-faucet) git repository with the Subnet's information. The following parameters are required.
...
Add the configuration in the array of `evmchains` inside the [config.json](https://github.com/luxdefi/lux-faucet/blob/main/config.json) file and make a PR.

## Building and Deploying a Faucet

You can also deploy and build your faucet by using the [Lux Faucet](https://github.com/luxdefi/lux-faucet) repository.
...
```bash
git clone https://github.com/luxdefi/lux-faucet
```
...
So, if you want to deploy this faucet, and have some reverse proxies in between, then you should configure this inside the `GLOBAL_RL` key of the `config.json` file. If this is not configured properly, then the users might get rate-limited very frequently, since the server-side proxy's IP addresses are being viewed as the client's IP. You can verify this in the code [here](https://github.com/luxdefi/lux-faucet/blob/23eb300635b64130bc9ce10d9e894f0a0b3d81ea/middlewares/rateLimiter.ts#L25).
...
Captcha is required to prove the user is a human and not a bot. For this purpose, we will use [Google's Recaptcha](https://www.google.com/recaptcha/intro/v3.html). The server side will require `CAPTCHA_SECRET` that should not be exposed. You can set the threshold score to pass the captcha test by the users [here](https://github.com/luxdefi/lux-faucet/blob/23eb300635b64130bc9ce10d9e894f0a0b3d81ea/middlewares/verifyCaptcha.ts#L20).
