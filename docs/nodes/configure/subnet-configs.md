...
Luxd looks for files specified with `{subnetID}.json` under
`--subnet-config-dir` as documented
[here](/nodes/configure/luxd-config-flags.md#subnet-configs).
...
same values used for the Primary Network, which are given [CLI Snow Parameters](/nodes/configure/luxd-config-flags.md#snow-parameters).
...
see [CLI Gossip Configs](/nodes/configure/luxd-config-flags.md#gossiping).
