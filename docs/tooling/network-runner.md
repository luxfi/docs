...
[Link to GitHub](https://github.com/luxdefi/network-runner)
...
You can also use the [Lux Network Runner Postman collection](https://github.com/luxdefi/network-runner-postman-collection).
...
```bash
curl -sSfL https://raw.githubusercontent.com/luxdefi/network-runner/main/scripts/install.sh | sh
```
...
requests through the `network-runner` command or by making API calls. Requests are
"translated" into gRPC and sent to the server.

Each node can then also be reached via
[API](https://github.com/luxdefi/network-runner/tree/main/api) endpoints which each node
exposes.
...
The network-runner supports LuxGo node configuration at different levels.
...
```bash
network-runner control remove-node node5 \
--request-timeout=3m \
--log-level debug \
--endpoint="0.0.0.0:8080" \
```
...
```bash
network-runner control restart-node node1 \
--request-timeout=3m \
--log-level debug \
--endpoint="0.0.0.0:8080" \
--luxd-path ${LUXD_EXEC_PATH}
...
```bash
network-runner control add-node node99 \
--request-timeout=3m \
--endpoint="0.0.0.0:8080" \
--luxd-path ${LUXD_EXEC_PATH}
```
...
The Lux Network Runner can also be imported as a library into your programs so that you can
...
[example](https://github.com/luxdefi/network-runner/blob/main/examples/local/fivenodenetwork/main.go).
...
Please refer to
[NetworkConfig](https://github.com/luxdefi/network-runner#network-creation) for more
details.
