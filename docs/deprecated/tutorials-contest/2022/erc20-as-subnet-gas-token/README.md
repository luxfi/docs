...
- General knowledge of [EthersJS](https://docs.ethers.io/v5/).
  - We will be interacting with both the Lux Testnet chain and our Subnet
...
the user. If you have used `--from lux` you will see the ERC20 balances of both
...
```bash
npx hardhat burnOrLock --from lux --amount <amount-of-token-to-burn-or-lock-in-ether>
```
...
When you try to run the first script `... --from lux --amount 10` if the user
...
- Lock 20 ERC20 token from LUX and see the updated balances of user and the bridge on LUX
- Relayer observes the transaction and sends a transaction to mint native tokens on Subnet
- Check the balances on both Subnet and LUX. As expected our Subnet native
...
- Relayer observers the transaction and sends a transaction to release 5 ERC20
  tokens on LUX
- Check balance on both Subnet and LUX. As expected our ERC20 balance increases
...
- Start by a lock transaction from LUX with amount 40. As stated in the video
...
- Check balances on both chains to confirm that old lock event on LUX has been
...
- Error while running scripts
  ...
  - If you are trying to bridge from Subnet to LUX, in other words trying to
...
Congratulations! You have created a bridge between LUX C-Chain and your Subnet
