---
description: This tutorial will show you how to setup a Chainlink node with the Lux Fuji Testnet and create dapp smart contracts utilizing a Chainlink node.
---
...
4. [LuxGo v1.4.5](https://github.com/luxdefi/luxd.git) or higher
5. [MetaMask wallet](https://metamask.io/) set up for use with Lux Fuji Testnet
...
is required for building LuxGo, later in this section.
...
The first step is to run LuxGo within docker which will map the TCP ports
9650 and 9651 in the container to the same ports on the Docker host.

Use the command below to run the LuxGo image within Docker:

```bash
docker run --name luxd-chainlink -d -p 9650:9650 -p 9651:9651 -v /root/.luxd:/root/.luxd avaplatform/luxd:91599fea /luxd/build/luxd --network-id=fuji --http-host=
```
...
- `/luxd/build/luxd --network-id=fuji` is the command to start the
  LuxGo under Fuji test network

Verify that the LuxGo node is started and running:

```bash
docker ps
```

This will list the LuxGo container status:
...
Now, in the jobs section, the newly created _Lux NodeJS adapter test_ will
...
MetaMask with Lux Fuji Testnet. Please have a look at this
[**section**](#deployed-smart-contracts) you can see our Lux Fuji
...
Add tokens to the Lux Fuji Testnet so that you can transfer some LINKs to
...
Use the [Lux Faucet](https://linkfaucet.protofire.io/fuji) and send some
...
Now, call the requestCurrentLuxSupply method on the deployed blockchain with
...
Then, click the supply button on the deployed contract it will return the current LUX supply value:
...
You know how to setup a Chainlink node with the Lux Fuji Testnet and
...
10. [LuxGo setup details](https://github.com/luxdefi/luxd)
