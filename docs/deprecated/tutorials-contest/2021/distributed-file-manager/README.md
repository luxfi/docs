# Distributed File Manager (DFM) using Lux, IPFS and ReactJS
...
**IPFS** protocol for storing our files, **Lux** network for storing the
...
the [Lux](https://lux.network) network, which is an instance of the EVM.
...
made using the **Solidity** language and will be deployed on the **Lux**
...
```bash
npx create-react-app dfm-react
```
...
`MetaMask` provider for interacting with the Lux network, `account`
...
deploy the smart contract on Lux's Fuji test network. This file is where
...
Lux's wallets like [https://wallet.lux.network](https://wallet.lux.network) use the path `m/44'/9000'/0'/0`
for its key derivation, since the coin type of Lux is **52752**. The list
...
## **Fund the Account and Run Migrations on Lux's Fuji Test Network.**

When deploying smart contracts to the Lux network, it will require some
deployment cost. As you can see inside `truffle-config.js`,
`@truffle/hdwallet-provider` will help us in deploying on Lux and
...
![lux c-chain address](./assets/distributed-file-manager-07-c-chain-address.png)
...
This might take a while depending upon your internet connection or traffic on the Lux network.
...
Lux's Fuji network, using their blockchain explorer
[here](https://testnet.luxscan.io/) by searching your transaction hash.
...
interact with the Lux blockchain are present in the `src` folder. First,
...
Now go to the project root directory of the project, the `dfm-react`
...
> this repository
> [https://github.com/rajranjan0608/dfm/tree/lux](https://github.com/rajranjan0608/dfm/tree/lux),
...
deploying the smart contract on **Lux** Fuji test network using
