# Tutorial: Lux Signature Verification in a dapp
...
the Lux Fuji test network (for this tutorial). You can add a few lines of
...
function checkMetamaskStatus() {
  if ((window as any).ethereum) {
    if ((window as any).ethereum.chainId != "0xa869") {
      result = "Failed: Not connected to Lux Fuji Testnet via MetaMask.";
    } else {
...
1. A human-readable part (HRP). On Lux Mainnet this is `lux` and on Fuji testnet it is `fuji`.
...
- prefix&mdash;Prefix for Bech32 addressing scheme (LUX or Fuji)
