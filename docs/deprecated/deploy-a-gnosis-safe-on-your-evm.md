...
programmatically on any [Subnet-EVM](/learn/lux/subnets-overview.md).
...
- A Subnet and EVM blockchain has been created. Lux tools allow users to do this on
  [Mainnet](/build/subnet/deploy/mainnet-subnet.md), [Testnet](/build/subnet/deploy/testnet-subnet.md)
...
EVM](https://docs.lux.network/dapps/smart-contracts/using-hardhat-with-the-c-chain#interact-with-smart-contract).
...
can send assets to your Safe the same way you would send LUX using
...
excerpt](/subnets/create-a-testnet-subnet#connect-with-metamask).
...
- `to` - Should be set to the proxy admin address found in
  `smart-contract-quickstart/.openzeppelin/"<YOUR-NETWORK-SESSION>".json`.
...
First, navigate back to hardhat console in your `smart-contract-quickstart-project` and
...
[`lux network status`](/build/subnet/deploy/local-subnet.md#checking-network-status). In this
