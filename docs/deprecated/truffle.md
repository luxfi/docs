...
pagination_label: Using Truffle with the Lux C-Chain
---
...
[Lux's architecture](/learn/lux/platform.md). You've also
...
- [Lux Network
  Runner](https://github.com/luxdefi/network-runner) is a tool for
...
[LuxJS](/tooling/luxjs-overview.md) which is used for cross chain
swaps.
...
API calls to a different LuxGo node. Also note that we're setting the
`gasPrice` and `gas` to the appropriate values for the Lux C-Chain.
...
Script will fund 1000 LUX to each account in `accounts` list above. After
...
[tutorial](https://support.lux.network/en/articles/6169872-how-to-make-a-cross-chain-transfer-in-the-wallet).
You'll need to send at least `135422040` nLUX to the account to cover the cost
...
`C-chain`/`Coreth` configs. The Lux Network Runner currently does not
