# Using Foundry with the Lux C-Chain
...
This section will walk you through creating an [ERC721](https://eips.ethereum.org/EIPS/eip-721) with Foundry and Lux Smart Contract Quickstart.
Clone the Lux Smart Contract Quickstart repo and install its dependencies by running:
...
In order to deploy contracts, you need to have some LUX. You can get testnet LUX from the [Lux Faucet](https://faucet.lux.network), which is an easy way to get to play around with Lux. After getting comfortable with your code, you can run it on Mainnet after making the necessary changes to your workflow.

## Implement a Game Item NFT

We will use our example ERC721 smart contract, [`NFT.sol`](https://github.com/luxdefi/lux-smart-contract-quickstart/blob/3fbba0ac28f6420e9be5d2635d5f23693f80127a/contracts/NFT.sol) found in `./contracts` of our project.
...
We can call functions on our NFT contract with [Cast](https://book.getfoundry.sh/reference/cast/cast-send.html), Foundry's command-line tool for interacting with smart contracts, sending transactions, and getting chain data. In this scenario, we will mint a Game Item to a player's wallet using the [`awardItem` function](https://github.com/luxdefi/lux-smart-contract-quickstart/blob/0f29cbb6375a1a452579213f688609c880d52c01/contracts/NFT.sol#L17) in our smart contract.
...
Next, deploy a new Lux Network with five nodes (a Cluster) locally.
...
:::warning
The example PRIVATE_KEY variable above provides a pre-funded account on Lux Network Runner and should be used for LOCAL DEVELOPMENT ONLY.
:::
