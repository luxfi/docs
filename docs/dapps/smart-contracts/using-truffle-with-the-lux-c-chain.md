# Using Truffle with the Lux C-Chain

## Introduction

[Truffle Suite](https://www.trufflesuite.com) is a toolkit for launching decentralized applications (dapps) on the EVM. With Truffle you can write and compile smart contracts, build artifacts, run migrations and interact with deployed contracts. This tutorial illustrates how Truffle can be used with Lux's C-Chain, which is an instance of the EVM.

## Requirements

You've completed [Run an Lux Node](../../nodes/build/run-lux-node-manually.md) and are familiar with [Lux's architecture](../../overview/getting-started/lux-platform.md). You've also performed a cross-chain swap via this [this tutorial](https://support.lux.network/en/articles/6169872-how-to-make-a-cross-chain-transfer-in-the-lux-wallet) to get funds to your C-Chain address.
...
A five node Lux network is running on your machine. Network will run until you CTRL + C to exit.
...
Use `npm` to install [web3](https://web3js.readthedocs.io), which is a library through which we can talk to the EVM and [LuxJS](../../apis/luxjs/README.md) which is used for cross chain swaps.

```text
npm install web3 lux -s
```
...
There is a convenient script that funds the `accounts` list . You can find it [here](https://raw.githubusercontent.com/luxdefi/lux-docs/master/scripts/fund-cchain-addresses.js). You can also download it using this command:

```text
wget -nd -m https://raw.githubusercontent.com/luxdefi/lux-docs/master/scripts/fund-cchain-addresses.js
```
...
Script will fund 1000 LUX to each account in `accounts` list above. After succesfully running the script you can check balances with:
...
If you wish to fund accounts your own, follow the steps in this [tutorial](https://support.lux.network/en/articles/6169872-how-to-make-a-cross-chain-transfer-in-the-lux-wallet). You'll need to send at least `135422040` nLUX to the account to cover the cost of contract deployments.
...
Now you have the tools you need to launch a local Lux network, create a truffle project, as well as create, compile, deploy and interact with Solidity contracts.
