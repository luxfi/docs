...
Not recommended for use on Mainnet. See warning notice in [Keystore API](/reference/luxd/keystore-api.md).

:::

Add a delegator to the Primary Network.

A delegator stakes LUX and specifies a validator (the delegatee) to validate on their behalf. The
...
us on [Discord.](https://chat.lux.network/)
...
Add a validator to the Primary Network. You must stake LUX to do this. If the node is sufficiently
...
This value will initially be set to `min(5 * amount staked, 3M LUX)`.
...
Send LUX from an address on the P-Chain to an address on the X-Chain or C-Chain. After issuing this
transaction, you must call the X-Chain's [`avm.import`](/reference/luxd/x-chain/api.md#avmimport)
or C-Chain's [`lux.import`](/reference/luxd/c-chain/api.md#luximport) with assetID `LUX` 
...
with [`platform.importKey`](/reference/luxd/p-chain/api.md#platformimportkey).
...
Before this method is called, you must call the X-Chain’s [`avm.export`](/reference/luxd/x-chain/api.md#avmexport)
or C-Chain’s [`lux.export`](/reference/luxd/c-chain/api.md#luxexport) method with assetID `LUX`
...
- `to` is the ID of the address the LUX is imported to. This must be the same as the `to` argument
