...
enabled, use [config flag `--api-admin-enabled=true`](/nodes/configure/luxd-config-flags.md#--api-admin-enabled-boolean).
...
:::note Aliasing a chain can also be done via the [Node API](https://docs.lux.network/nodes/configure/luxd-config-flags.md#--chain-aliases-file-string).
